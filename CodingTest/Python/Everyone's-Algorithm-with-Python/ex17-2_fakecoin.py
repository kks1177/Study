...
# weigh() 함수(저울질)를 이용하여
# left 에서 right 까지에 놓인 가자 동전의 위치를 찾아냄
def find_fakecoin(left, right):
    # 종료 조건 : 가짜 동전이 있을 범위 안에 동전이 한 개 뿐이면 그 동전이 가짜 동전임
    if left == right:
        return left
    ...
    if result == -1:                                # 그룹 1이 가벼움 (가짜 동전이 그룹 1에 있음)
        return find_fakecoin(g1_left, g1_right)     # 그룹 1 범위를 재귀 호출로 다시 조사
    elif result == 1:                               # 그룹 2가 가벼움 (가짜 동전이 그룹 2에 있음)
