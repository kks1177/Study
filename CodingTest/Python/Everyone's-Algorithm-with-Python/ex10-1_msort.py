...
def merge_sort(a):
    ...
    result = []         # 두 그룹을 합쳐 만들 최종 결과 리스트
    
    # 두 그룹을 하나로 병합
    while g1 and g2:    # 두 그룹에 모두 자료가 남아 있는 동안 반복
        if g1[0] < g2[0]:               # 두 그룹의 맨 앞 자료 값을 비교
            # g1 그룹 값이 더 작으면 그 값을 빼내어 결과 리스트에 추과
            result.append(g1.pop(0))
        else:
            # g2 그룹 값이 더 작으면 그 값을 빼내어 결과 리스트에 추과
            result.append(g2.pop(0))
    
    # 두 그룹 비교 과정이 끝났을 때 아직 그룹에 남아 있는 자료들을 결과 리스트에 추가
    # g1과 g2 그룹 중 이미 빈 것은 while 을 거치지 않고 지나감
    while g1:
        result.append(g1.pop(0))
    while g2:
        result.append(g2.pop(0))
