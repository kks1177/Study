...
class Rectangle(Shape):     # 서브 클래스
    ...
    def __init__(self, x1, y1, x2, y2, c, w):
        ...
        self.width = W
        self.drawShape()
    def __del__(self):      # 사각형 선분 4개를 삭제
...
def getColor():      # 임의의 색상 선택
    r = random.randrange(16, 256)   # 16진수로 변환하는 0~A는 제외
    g = random.randrange(16, 256)   # 16진수로 변환하는 0~A는 제외
    b = random.randrange(16, 256)   # 16진수로 변환하는 0~A는 제외
...
def endDrawCircle(event):
    ...
    x2 = event.x
    y2 = event.y
    cir = Circle(x1, y1, x2, y2, getColor(), getWidth())        # 사각형 생성
...
## 전역 변수 선언 부분 ##
shapes = []     # 화면에 그린 전체 도형 리스트
window = None
canvas = None
x1, y1, x2, y2 = [None] * 4     # 클릭한 두 지점의 X, Y
